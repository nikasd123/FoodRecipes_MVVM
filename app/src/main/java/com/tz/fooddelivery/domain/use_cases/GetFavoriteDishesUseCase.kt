...
@Singleton
class GetFavoriteDishesUseCase @Inject constructor(
    private val mealsRepository: MealsRepository
) {
    suspend fun getFavoriteDishes() : Flow<Result<List<DishItem>, NetworkError>>{
        return mealsRepository.getFavoriteDishes().map { result ->
            when (result){
